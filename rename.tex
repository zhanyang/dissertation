...
on \bets that makes efficient renames possible on full-path-indexed \betrfs.
...
Define a key/value pair whose key has prefix \spre as a source key/value pair
and a key/value pair whose key has prefix \dpre as a destination key/value pair.
...
In such scenario, splitting the root node divides the tree into a forest.
...
\Fti performs multiple operations atomically in a transaction.

When a transaction commits or aborts, \fti injects one transaction-commit or
transaction-abort message for each operations in the transaction.
