...
At last, Section~\ref{sec:rr:impl} talks about implementation details of
...
which contains and only contains all keys of a certain prefix,
in the \bet through \textbf{tree surgery}.
...
Therefore, to generate the smallest subtree of range $(p_{min}, p_{max})$,
tree surgery slices out the subtree rooted at the LCA.
...
Because now keys in Node $B$ don't have prefix ``/R/'' physically in the node,
...
i.e., non-leaf nodes without enough children or leaf nodes without enough
key/value pairs.
...
in which the node being split can be nearly undersized and
the two slicing keys can split the node unevenly.
Therefore, tree surgery can generate nodes that are undersized.
Moreover, to make the source and destination subtree at the same height,
tree surgery keeps slicing the subtree with a lower LCA,
which creates empty non-leaf nodes with only one child.
...
the readers-writer locks of \bet nodes (Section~\ref{sec:bg:impl:sync}).
...
After crash, the range-rename operation can be recovered if the log entry is
