...
We run each benchmark on a file system twice, one on an empty file system,
the other on an aged file system.
We age the file system by emulating user behaviors on the file system.
First, we fill the file system with files and directories from the root file
system, roughly taking up 31 GiB space.
Then, we clone a the git repository of the Linux kernel with the release tag
``v3.12'' to the home directory in the file system.
Afterwards, we repeat the process of building the kernel and pull the next
release of the repository
until we build the Linux kernel with the release tag ``v4.20''.
This process builds the kernel 240 times
(though there are only 29 major releases, there are 7 to 9 release candidates
between two consecutive major releases)
and pulls 397481 git commits.
The git repository itself takes up about 3.6 GiB space and
after building the kernel, the directory takes up about 15 GiB space.
...
\begin{figure}[t]
    \begin{subfigure}{\textwidth}
        ...
        \begin{tikzpicture}[yscale=0.95, xscale=0.95]
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Benchmarking on empty file systems.}
    \end{subfigure}
    \begin{subfigure}{\textwidth}
        \centering
        \begin{tikzpicture}[yscale=0.95, xscale=0.95]
            ...
            \end{axis}
        \end{tikzpicture}
        \caption{Benchmarking on aged file systems.}
    \end{subfigure}
