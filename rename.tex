...
To perform a rename, \betrfs simply calls the range-rename operation,
which updates all related keys in the underlying \bets efficiently.
This chapter starts by describing the range-rename interface,
then explains how the range-rename operation updates a large amount of keys
efficiently through two techniques, tree surgery and key lifting.

\section{Range-rename interface}

range-rename(old\_prefix, new\_prefix)
