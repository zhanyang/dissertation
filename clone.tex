...
\paragraph{Flushing a \goto message to a node higher than its source LCA.}
...
The lifted \bedag may need to merge children before flushing the \goto message
When flushing a \goto message from a node, the key range of the \goto message
$(\dpre_{min}, \dpre_{max})$ might overlap with multiple children of the node.
The lifted \bedag can merge all the related children to generate a child that
can accommodate the \goto message.
In fact, because the \goto message nullifies old messages with destination keys,
all interior children (an interior node is a node whose key range falls
completely in the key range of the \goto message) can be discarded.
Therefore, the lifted \bedag only needs to merge two fringe children and sends
all interior children to the background garbage collection thread.
...
There are some special cases when creating a child to accommodate the \goto
message.
Specifically, if one of \dpre$_{min}$ and \dpre$_{max}$ matches a pivot in the
node, there is only one fringe child.
The lifted \bedag then discards all interior children and expands the range of
the fringe child, done by merging the fringe child with an empty node.
If both \dpre$_{min}$ and \dpre$_{max}$ are pivots in the node, all children
are interior nodes.
In this scenario, the lifted \bedag keeps one interior child and expands the
range of the child.

\paragraph{Flushing a \goto message to a node at the same height as its source LCA.}
